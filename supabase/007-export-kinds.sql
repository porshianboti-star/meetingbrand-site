...
alter table public.backgrounds
  add column if not exists thumb_asset_id      uuid references public.brand_assets(id) on delete set null,  -- 280x158 PNG (Teams tile), bucket exports
  add column if not exists export_jpg_asset_id uuid references public.brand_assets(id) on delete set null;  -- 1920x1080 JPEG (Meet console), bucket exports
...
create policy brand_assets_insert on public.brand_assets
  for insert with check (
    ...
    and (public.my_role() = 'admin' or kind in ('scene','export','preview','thumb','export_jpg'))
