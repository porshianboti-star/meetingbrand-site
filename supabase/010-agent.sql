...
create index if not exists enrollment_keys_org_idx on public.enrollment_keys (org_id);

-- ---------- agents ----------
create table if not exists public.agents (
  id               uuid primary key default gen_random_uuid(),
  org_id           uuid not null references public.orgs(id) on delete cascade,
  employee_id      uuid references public.employees(id) on delete set null,   -- null = unmatched (admin binds later)
...
create index if not exists agents_employee_idx on public.agents (employee_id);

-- an agent may only be bound to an employee of ITS OWN org (even by the service role)
create or replace function public.guard_agent_consistency()
...
drop trigger if exists agents_guard_consistency on public.agents;
create trigger agents_guard_consistency
  before insert or update of org_id, employee_id on public.agents
  for each row execute function public.guard_agent_consistency();
...
create index if not exists agent_reports_agent_idx on public.agent_reports (agent_id, created_at desc);
create index if not exists agent_reports_push_idx  on public.agent_reports (push_id);
...
grant select, insert, update, delete on public.enrollment_keys, public.agents, public.agent_reports to service_role;
grant usage, select on sequence public.agent_reports_id_seq to service_role;

drop policy if exists enrollment_keys_admin_select on public.enrollment_keys;
create policy enrollment_keys_admin_select on public.enrollment_keys
...
create or replace view public.enrollment_keys_view with (security_invoker = true) as
  select id, org_id, label, created_by, created_at, revoked_at, last_used_at, uses
  from public.enrollment_keys;
create or replace view public.agents_view with (security_invoker = true) as
  select id, org_id, employee_id, device_id, os, hostname, local_identity, version,
         token_expires_at, enrolled_at, last_seen_at, revoked_at
  from public.agents;
revoke all on public.enrollment_keys_view, public.agents_view from anon, authenticated, service_role;
grant select on public.enrollment_keys_view, public.agents_view to authenticated, service_role;
...
create or replace function public.create_enrollment_key(p_label text default null)
returns jsonb
...
begin
  ...
  select count(*) into v_active from public.enrollment_keys where org_id = v_org and revoked_at is null;
  ...
  insert into public.enrollment_keys (org_id, key_hash, label, created_by)
...
revoke all on function public.create_enrollment_key(text) from public, anon;
grant execute on function public.create_enrollment_key(text) to authenticated;

create or replace function public.revoke_enrollment_key(p_id uuid)
returns boolean
language plpgsql security definer set search_path = public as $$
...
begin
  if v_org is null then raise exception 'not authenticated'; end if;
  if public.my_role() <> 'admin' then raise exception 'admin only'; end if;
  update public.enrollment_keys set revoked_at = now()
   where id = p_id and org_id = v_org and revoked_at is null;
  return found;
end $$;
revoke all on function public.revoke_enrollment_key(uuid) from public, anon;
grant execute on function public.revoke_enrollment_key(uuid) to authenticated;
...
create or replace function public.bind_agent(p_agent_id uuid, p_employee_id uuid)
returns boolean
language plpgsql security definer set search_path = public as $$
declare v_org uuid := public.my_org();
begin
  ...
  if p_employee_id is not null and not exists
     (select 1 from public.employees e where e.id = p_employee_id and e.org_id = v_org) then
    raise exception 'employee must belong to this workspace';
  end if;
  update public.agents set employee_id = p_employee_id
   where id = p_agent_id and org_id = v_org;
  return found;
end $$;
revoke all on function public.bind_agent(uuid, uuid) from public, anon;
grant execute on function public.bind_agent(uuid, uuid) to authenticated;
...
create or replace function public.enrollment_key_use(p_key_hash text, p_limit int default 60)
returns table (key_id uuid, key_org_id uuid, revoked boolean, limited boolean)
language plpgsql set search_path = public as $$
declare
  r public.enrollment_keys%rowtype;
begin
  select * into r from public.enrollment_keys k where k.key_hash = p_key_hash for update;
  ...
  update public.enrollment_keys
     set window_start = r.window_start, window_uses = r.window_uses + 1,
         uses = public.enrollment_keys.uses + 1, last_used_at = now()
   where id = r.id;
  return query select r.id, r.org_id, false, false;
end $$;
revoke all on function public.enrollment_key_use(text, int) from public, anon, authenticated;
grant execute on function public.enrollment_key_use(text, int) to service_role;
