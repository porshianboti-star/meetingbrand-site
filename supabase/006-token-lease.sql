...
create or replace function public.integration_token_begin_refresh(p_integration_id uuid, p_lease_seconds int default 30)
...
revoke all on function public.integration_token_begin_refresh(uuid, int) from public, anon, authenticated;
grant execute on function public.integration_token_begin_refresh(uuid, int) to service_role;

-- ---------- end: release the lease without writing tokens (refresh failed / not needed) ----------
create or replace function public.integration_token_end_refresh(p_integration_id uuid)
returns void
language plpgsql set search_path = public as $$
begin
  update priv.integration_tokens set refresh_lease_until = null where integration_id = p_integration_id;
end $$;
revoke all on function public.integration_token_end_refresh(uuid) from public, anon, authenticated;
grant execute on function public.integration_token_end_refresh(uuid) to service_role;
