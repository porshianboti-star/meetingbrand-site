...
create index if not exists events_event_idx   on public.events (product, event, created_at);
...
create or replace function public.funnel_stats(p_days int)
...
language sql stable security definer set search_path = public as $$
  ...
  order by e.event
$$;
grant execute on function public.funnel_stats(int) to anon, authenticated;
