...
//
//   POST /enroll      {orgKey, deviceId, os: windows|macos, hostname?, localIdentity?, version?, employeeEmail?}
...
//                     401 bad key · 410 revoked key · 429 rate-limited
//   GET  /assignments Bearer <deviceToken>
...
//   POST /report      Bearer  {items:[{backgroundId, state: written|verified|restart_needed|removed|blocked|error, evidence?}]}
...
//                       / push_removed {platform:'teams', via:'agent'}; an item for a pair that was never assigned is
//                       rejected per item; 422 when nothing in the batch was accepted
...
export function makeHandler(deps: Deps = {}) {
  ...
  return serveFn("mb-agent", async (req, { log }) => {
    ...
    if (action === "enroll") {
      ...
      if (!KEY_RE.test(orgKey)) throw new HttpError(401, "bad_key", "orgKey must be an enrollment key (mbk_…)");
      ...
      if (os !== "windows" && os !== "macos") throw new HttpError(400, "bad_request", "os must be windows or macos");
      ...
      if (!key) {
        log.warn("enroll_bad_key", { device_id: deviceId });
        throw new HttpError(401, "bad_key", "unknown enrollment key");
      }
      if (key.revoked) throw new HttpError(410, "key_revoked", "this enrollment key was revoked — ask the workspace admin for a new one");
    ...
    if (action === "assignments") {
      ...
      if (ids.length) {
        const { data, error } = await sb.from("backgrounds").select("id, label, slug, export_asset_id, thumb_asset_id").eq("org_id", orgId).in("id", ids);
        if (error) throw new HttpError(500, "db_error", `backgrounds lookup failed: ${error.message}`);
        backgrounds = (data ?? []) as BackgroundRow[];
      }
      const { data: pushRows, error: pErr } = await sb.from("pushes").select("id, background_id, state").eq("org_id", orgId).eq("employee_id", employee.id).eq("platform", "teams");
      ...
      for (const bg of shaped.write) {
        ...
        let sha = png.sha256 && /^[0-9a-f]{64}$/i.test(png.sha256) ? png.sha256.toLowerCase() : null;
        let bytes = png.bytes;
        if (!sha) {
          const { data: blob, error: dlErr } = await sb.storage.from(EXPORT_BUCKET).download(png.storage_path);
          if (dlErr || !blob) {
            log.warn("export_download_failed", { org_id: orgId, asset_id: png.id, message: dlErr?.message ?? "empty" });
            notReady.push(bg.id);
            continue;
          }
          ...
          const { error: upErr } = await sb.from("brand_assets").update({ sha256: sha, bytes }).eq("id", png.id);
          if (upErr) log.warn("sha_store_failed", { asset_id: png.id, message: upErr.message });
        }
        const { data: signed, error: sErr } = await sb.storage.from(EXPORT_BUCKET).createSignedUrl(png.storage_path, SIGNED_URL_TTL_S);
        if (sErr || !signed?.signedUrl) {
          log.warn("sign_failed", { asset_id: png.id, message: sErr?.message ?? "no url" });
          notReady.push(bg.id);
          continue;
        }
      ...
      if (shouldRotate(agent.token_expires_at, now())) {
        const t = newDeviceToken();
        patch.token_hash = await sha256Hex(t);
        patch.token_expires_at = new Date(now() + TOKEN_TTL_MS).toISOString();
      ...
      log.info("assignments", { org_id: orgId, agent_id: agent.id, employee_id: employee.id, write: handed.size, remove: shaped.remove.length, skipped: shaped.skipped.length + notReady.length, queued: queue.length, rotated: !!rotated });
      return json(req, 200, {
        agentId: agent.id,
    ...
    if (action === "report") {
      ...
      for (const raw of body.items as unknown[]) {
        ...
        if (!UUID_RE.test(bgId) || !(REPORT_STATES as readonly string[]).includes(state)) {
          results.push({ backgroundId: bgId || "?", ok: false, error: `each item needs a uuid backgroundId and a state in ${REPORT_STATES.join("|")}` });
          continue;
        }
        const evidence = cleanEvidence(it.evidence);
        const { data: pr, error: prErr } = await sb.from("pushes").select("id, state").eq("idempotency_key", pushKey(orgId, agent.employee_id, bgId)).eq("org_id", orgId).limit(1);
        ...
        const { error: updErr } = await sb.from("pushes").update({
          ...
          error: mapped.error,
          agent_id: agent.id,
          evidence: { ...(evidence ?? {}), via: "agent", agent_id: agent.id, os: agent.os, agent_version: agent.version, report: state, reported_at: nowIso },
        }).eq("id", push.id);
        if (updErr) throw new HttpError(500, "db_error", `pushes update failed: ${updErr.message}`);
        await recordEvent(sb, mapped.event, null, { platform: "teams", via: "agent", org_id: orgId, agent_id: agent.id, employee_id: agent.employee_id, background_id: bgId, report: state, state: mapped.pushState });
