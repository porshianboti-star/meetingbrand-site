...
| `009-employees-insert-guard.sql` | **yes — applied 2026-09-16** (as `shared-cc/mb-009-employees-insert-guard.sql`; trigger `employees_guard_insert` verified live with a QA admin, deleted afterwards) | security review: BEFORE INSERT twin of `guard_employee_columns` — client inserts are `platform='csv'` only, `synced_at` forced null, text caps (ext_id/email 320, name/title/dept 200), `assigned_bg` must be this org's backgrounds, ≤ 5000 CSV rows per org; service role untouched |
| `010-agent.sql` | **yes — applied 2026-09-17** (as `shared-cc/mb-010-agent.sql`, via the Management API, with the Edge Function `mb-agent`; smoke-tested with a QA admin + a QA CSV employee, deleted afterwards, 0 residue) | the MeetingBrand Agent backend: `enrollment_keys` (sha256 of the `mbk_…` org key, shown once by `create_enrollment_key(p_label)`), `agents` (one per org+device, sha256 of the `mbd_…` device token, 90-day expiry), `agent_reports`; `pushes.agent_id`; a `teams` push may target an employee row of any platform; admins read `enrollment_keys_view` / `agents_view` (no hashes) and call `revoke_enrollment_key` / `revoke_agent` / `bind_agent`; `enrollment_key_use` (service role) = key lookup + revocation + 60/min rate window |
| `011-extension.sql` | **yes — applied 2026-09-17** (as `shared-cc/mb-011-extension.sql`, via the Management API, with the redeployed Edge Function `mb-agent`) | the Meet browser extension enrolls like an agent: `agents.os` += `chrome` / `edge`; a `meet` push may target an employee row of any platform (like `teams` since 010); `agent_reports.state` += `applied` / `unavailable` (the extension's vocabulary; `error` shared); `revoke_agent` also fails the device's `meet` pushes. mb-agent: `GET /assignments?platform=meet` (default for chrome/edge devices) hands out ONE look (JPEG `export_jpg_asset_id` preferred, else the PNG export) as `action:'apply'`; `/report` maps applied → `selected`, unavailable → `pushed` + reason, error → `failed` (events `via:'extension'`) |
| `012-zoom-app.sql` | **yes — applied 2026-09-23** (as `shared-cc/mb-012-zoom-app.sql`, via the Management API, with the deployed Edge Functions `mb-agent` + the new `mb-zoom-app`) | the user-managed Zoom App ("MeetingBrand for Zoom", `zoom-app/README-ZOOM-APP.md`) enrolls like an agent WITHOUT an org key: `agents.os` += `zoom`; a `zoom` push may target an employee row of any platform; `agent_reports.state` += `denied`; `revoke_agent` also fails the device's `zoom` pushes. mb-agent: `POST /zoom/resolve {ticket}` (the ticket = the decrypted `x-zoom-app-context` Zoom user id, minted by mb-zoom-app) → device token when that Zoom user is a synced `employees` row (platform `zoom`, `ext_id` = uid) of a workspace whose Zoom integration is connected; `GET /assignments?platform=zoom` hands out ONE look (JPEG preferred; push key `…:zoom`, distinct from mb-push-zoom's REST rows); `/report` maps applied → `selected`, denied → `awaiting_client` + reason, error → `failed` (events `via:'zoom_app'`) |
