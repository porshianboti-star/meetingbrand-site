...
create or replace function mb.delete_my_account()
returns void
...
begin
  ...
  select org_id into v_org from mb.profiles where id = v_uid;
  update mb.events set user_id = null where user_id = v_uid;   -- keep the funnel, drop the identity
  ...
  if v_org is not null and not exists
     (select 1 from mb.profiles where org_id = v_org) then
    delete from mb.orgs where id = v_org;  -- cascades: brands, brand_assets, backgrounds, invites
  end if;
end $$;
revoke all on function mb.delete_my_account() from public, anon;
grant execute on function mb.delete_my_account() to authenticated;

-- ---------- owner: list all orgs ----------
-- Shape kept close to PS owner_list_accounts so the back office swaps only
-- `signatures` → `backgrounds` (company = orgs.name).
create or replace function mb.owner_list_accounts(k text)
returns jsonb
language plpgsql security definer set search_path = mb, public as $$
begin
  ...
  return coalesce((
    select jsonb_agg(jsonb_build_object(
      ...
      'backgrounds', (select count(*) from mb.backgrounds b where b.org_id = o.id),
      'brand_source', (select br.source from mb.brands br
                       where br.org_id = o.id and br.is_active limit 1),
      'users', coalesce((select jsonb_agg(jsonb_build_object(
                 'id', p.id, 'email', p.email,
                 'role', p.role, 'name', p.full_name, 'created_at', p.created_at))
               from mb.profiles p where p.org_id = o.id),
               '[]'::jsonb)
    ) order by o.created_at desc)
    from mb.orgs o), '[]'::jsonb);
end $$;
revoke all on function mb.owner_list_accounts(text) from public, anon;
grant execute on function mb.owner_list_accounts(text) to authenticated;

-- ---------- owner: delete a whole tenant ----------
create or replace function mb.owner_delete_account(k text, acc uuid)
returns integer
language plpgsql security definer set search_path = mb, public as $$
declare n integer;
begin
  perform priv.check_owner_key(k);
  select count(*) into n from mb.profiles where org_id = acc;
  update mb.events set user_id = null
   where user_id in (select id from mb.profiles where org_id = acc);
  delete from auth.users
   where id in (select id from mb.profiles where org_id = acc);
  delete from mb.orgs where id = acc;
  return n;
end $$;
revoke all on function mb.owner_delete_account(text, uuid) from public, anon;
grant execute on function mb.owner_delete_account(text, uuid) to authenticated;
...
create or replace function mb.signup_export(k text)
returns table (
  admin_email  text,
  company_name text,
  domain       text,
...
language plpgsql stable security definer set search_path = mb, public as $$
begin
  perform priv.check_owner_key(k);
  return query
  ...
  select fa.email,
         ...
         o.plan,
         o.created_at,
         (select count(*) from mb.backgrounds b where b.org_id = o.id),
         (select count(*) from mb.profiles   p where p.org_id = o.id),
         (select br.source from mb.brands br where br.org_id = o.id and br.is_active limit 1)
  from mb.orgs o
...
revoke all on function mb.signup_export(text) from public, anon;
grant execute on function mb.signup_export(text) to authenticated;
-- the pre-review, ungated zero-argument version must not survive on a project that ran the old 003
drop function if exists mb.signup_export();
...
create or replace function mb.owner_set_plan(k text, acc uuid, p_plan text)
returns text
language plpgsql security definer set search_path = mb, public as $$
begin
  perform priv.check_owner_key(k);
  if p_plan not in ('free','pro','business') then raise exception 'unknown plan'; end if;
  update mb.orgs set plan = p_plan where id = acc;
  if not found then raise exception 'workspace not found'; end if;
  return p_plan;
end $$;
revoke all on function mb.owner_set_plan(text, uuid, text) from public, anon;
grant execute on function mb.owner_set_plan(text, uuid, text) to authenticated;
