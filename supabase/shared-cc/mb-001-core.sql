...
grant usage on schema mb to anon, authenticated, service_role;
alter default privileges in schema mb grant select, insert, update, delete on tables to anon, authenticated, service_role;
alter default privileges in schema mb grant usage, select on sequences to anon, authenticated, service_role;
-- ============================================================
...
-- ---------- Tables ----------
create table if not exists mb.orgs (
...
create index if not exists orgs_domain_idx on mb.orgs (domain);

create table if not exists mb.profiles (
...
create index if not exists profiles_org_idx on mb.profiles (org_id);

create table if not exists mb.invites (
  id          uuid primary key default gen_random_uuid(),
  org_id      uuid not null references mb.orgs(id) on delete cascade,
...
create index if not exists invites_org_idx on mb.invites (org_id);
-- idempotent repair of the FK for a project created before the review (no-op on a fresh one)
alter table mb.invites drop constraint if exists invites_accepted_by_fkey;
alter table mb.invites add constraint invites_accepted_by_fkey
...
create table if not exists mb.free_mail_domains (
  domain text primary key
);
insert into mb.free_mail_domains (domain) values
...
create or replace function mb.my_org() returns uuid
language sql stable security definer set search_path = mb, public as
$$ select org_id from mb.profiles where id = auth.uid() $$;

create or replace function mb.my_role() returns text
language sql stable security definer set search_path = mb, public as
$$ select role from mb.profiles where id = auth.uid() $$;
...
create or replace function mb.is_trusted() returns boolean
language sql stable set search_path = mb, public as
$$ select current_user in ('postgres','service_role','supabase_admin')
       or coalesce(auth.role(), '') = 'service_role' $$;

create or replace function mb.is_free_mail(p_domain text) returns boolean
language sql stable set search_path = mb, public as
$$ select exists (select 1 from mb.free_mail_domains where domain = lower(p_domain)) $$;
...
create or replace function mb.clean_domain(p_raw text) returns text
language sql immutable set search_path = mb, public as
$$
  select case
...
create or replace function mb.guard_profile_columns()
returns trigger language plpgsql set search_path = mb, public as $$
begin
  if mb.is_trusted() then
    return new;
  end if;
  ...
  if new.role is distinct from old.role then
    if not (mb.my_role() = 'admin' and old.org_id = mb.my_org() and old.id <> auth.uid()) then
...
drop trigger if exists profiles_guard_columns on mb.profiles;
create trigger profiles_guard_columns
  before update on mb.profiles
  for each row execute function mb.guard_profile_columns();
...
create or replace function mb.guard_org_plan()
returns trigger language plpgsql set search_path = mb, public as $$
begin
  if new.id is distinct from old.id then
...
drop trigger if exists orgs_guard_plan on mb.orgs;
create trigger orgs_guard_plan
  before update on mb.orgs
  for each row execute function mb.guard_org_plan();

-- ---------- Row Level Security ----------
alter table mb.orgs              enable row level security;
alter table mb.profiles          enable row level security;
alter table mb.invites           enable row level security;
alter table mb.free_mail_domains enable row level security;

-- orgs: members read their own org; admin may rename / change settings / domain
drop policy if exists orgs_select on mb.orgs;
create policy orgs_select on mb.orgs
  for select using (id = mb.my_org());
drop policy if exists orgs_update on mb.orgs;
create policy orgs_update on mb.orgs
  for update using (id = mb.my_org() and mb.my_role() = 'admin')
  with check (id = mb.my_org() and mb.my_role() = 'admin');

-- profiles: user sees own; admin sees all of own org
drop policy if exists profiles_select on mb.profiles;
create policy profiles_select on mb.profiles
  for select using (
    id = auth.uid()
    or (org_id = mb.my_org() and mb.my_role() = 'admin')
  );
drop policy if exists profiles_update_self on mb.profiles;
create policy profiles_update_self on mb.profiles
  for update using (id = auth.uid())
  with check (id = auth.uid());              -- columns further restricted by profiles_guard_columns
drop policy if exists profiles_update_admin on mb.profiles;
create policy profiles_update_admin on mb.profiles
  for update using (org_id = mb.my_org() and mb.my_role() = 'admin')
  with check (org_id = mb.my_org());
drop policy if exists profiles_delete_admin on mb.profiles;
create policy profiles_delete_admin on mb.profiles
  for delete using (
    org_id = mb.my_org() and mb.my_role() = 'admin' and id <> auth.uid()
  );

-- invites: only admins of the org manage them (CC invites_admin_all)
drop policy if exists invites_admin_all on mb.invites;
create policy invites_admin_all on mb.invites
  for all using (org_id = mb.my_org() and mb.my_role() = 'admin')
  with check (org_id = mb.my_org() and mb.my_role() = 'admin');

-- free_mail_domains: readable by everyone (the client may sync its GENERIC list), never writable
drop policy if exists free_mail_domains_read on mb.free_mail_domains;
create policy free_mail_domains_read on mb.free_mail_domains
...
create or replace function mb.handle_new_user()
returns trigger language plpgsql security definer set search_path = mb, public as $$
declare
  v_org         uuid;
  v_role        text := 'member';
  v_token       uuid;
  v_invite      mb.invites%rowtype;
  v_email       text := lower(coalesce(new.email, ''));
  v_mail_domain text := nullif(split_part(lower(coalesce(new.email, '')), '@', 2), '');
  v_meta_domain text := mb.clean_domain(new.raw_user_meta_data->>'domain');
...
begin
  if coalesce(new.raw_user_meta_data->>'product', '') <> 'meetingbrand' then
    return new;                                   -- not a MeetingBrand signup (CompanyCard user): nothing to do here
  end if;
  if coalesce(new.raw_user_meta_data->>'invite_token', '') <> '' then
    ...
    select * into v_invite from mb.invites
    ...
    if v_mail_domain is not null and not mb.is_free_mail(v_mail_domain) then
      v_domain := v_mail_domain;
    elsif v_meta_domain is not null and not mb.is_free_mail(v_meta_domain) then
      v_domain := v_meta_domain;
    else
      v_domain := null;
    end if;
    insert into mb.orgs (name, domain)
  ...
  insert into mb.profiles (id, org_id, role, full_name, email)
  values (new.id, v_org, v_role, v_full_name, new.email);
  return new;
end $$;

revoke all on function mb.handle_new_user() from public, anon, authenticated;
grant execute on function mb.handle_new_user() to supabase_auth_admin;

drop trigger if exists mb_on_auth_user_created on auth.users;
create trigger mb_on_auth_user_created
  after insert on auth.users
  for each row execute function mb.handle_new_user();

-- ---------- Invite lookup for the gate (anonymous, by token only) ----------
create or replace function mb.invite_info(p_token uuid)
returns table (org_name text, email text, role text)
language sql stable security definer set search_path = mb, public as $$
  select o.name, i.email, i.role
  from mb.invites i join mb.orgs o on o.id = i.org_id
  where i.token = p_token and i.accepted_at is null and i.expires_at > now()
$$;
grant execute on function mb.invite_info(uuid) to anon, authenticated;
...
revoke all on function mb.set_org_domain(text) from public, anon;
grant execute on function mb.set_org_domain(text) to authenticated;
...
create or replace function mb.org_for_domain(p_domain text)
...
language sql stable security definer set search_path = mb, public as $$
  ...
  select o.id, o.name,
         (select count(*) from mb.profiles p where p.org_id = o.id),
         coalesce(o.settings->>'domain_join', 'invite')
  from mb.orgs o, me
  where o.domain is not null
    and o.domain = mb.clean_domain(p_domain)
    and o.domain = me.domain
    and not mb.is_free_mail(o.domain)
    and o.id is distinct from mb.my_org()
    and exists (select 1 from mb.profiles p join auth.users u on u.id = p.id
                where p.org_id = o.id
                  and split_part(lower(coalesce(u.email, '')), '@', 2) = o.domain)
  order by 3 desc, o.created_at
$$;
revoke all on function mb.org_for_domain(text) from public, anon;
grant execute on function mb.org_for_domain(text) to authenticated;
...
create or replace function mb.join_org_by_domain(p_org_id uuid)
returns void
language plpgsql security definer set search_path = mb, public as $$
declare
  v_uid         uuid := auth.uid();
  v_old_org     uuid := mb.my_org();
  v_mail_domain text;
  v_target      mb.orgs%rowtype;
begin
  ...
  select * into v_target from mb.orgs where id = p_org_id;
  ...
  if v_target.domain is null or v_mail_domain is null or v_mail_domain = ''
     or v_target.domain <> v_mail_domain or mb.is_free_mail(v_target.domain) then
  ...
  if (select count(*) from mb.profiles where org_id = v_old_org) <> 1 then
    raise exception 'leave your current workspace first';
  end if;
  update mb.profiles set org_id = v_target.id, role = 'member' where id = v_uid;
  delete from mb.orgs where id = v_old_org;   -- cascades brands/backgrounds/invites of the empty org
end $$;
revoke all on function mb.join_org_by_domain(uuid) from public, anon;
grant execute on function mb.join_org_by_domain(uuid) to authenticated;
